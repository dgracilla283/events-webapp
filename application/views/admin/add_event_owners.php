<form class="form-horizontal" method="post" action="/admin/add_event_owners" id="form-add-event-owners">
<fieldset>
	...
	<ul id="attendees" class="attendees">
		...
		<li class="user-block clearfix">
			<a href="javascript:;" class="add-user-btn<?php echo (array_key_exists($user['userID'],$event_owners)) ? ' selected' : '';?>" data-userid="<?php echo $user['userID'];?>" title="<?php echo $user['last_name'].', '.$user['first_name']; ?>">
			...
			<?php if(array_key_exists($user['userID'],$event_owners)): ?>
