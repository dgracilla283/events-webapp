$(document).on("pageinit", function(e){
	...
	$('.slidemenu_btn').on('click', function(e) {
		$('.ui-loader').hide();
	});
...
function slidemenu(sm, only_close) {
	...
	if (!sm.data('slideopen') && !only_close) {
		...
		if ($(".ui-page-active :jqmData(role='header')").data('position') == 'fixed') {
			$("#header-inner").find(".header-inner-content").css('margin-left', '240px');
		...
		$("#header-inner").find(".header-inner-content").css('margin-left', '0px');
	}
	return false;
}
